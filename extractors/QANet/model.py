...
from extractors.QANet.layers import initializer, regularizer, residual_block, highway, conv, mask_logits, trilinear, total_params


class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True, infer=True, graph=None):
        self.config = config
        self.infer = infer
        ...
        with self.graph.as_default():
            ...
            if opt:
                N, CL = config["batch_size"] if not self.infer else 1, config["char_limit"]
    ...
    def forward(self):
        config = self.config
        N, PL, QL, CL, d, dc, nh = config["batch_size"] if not self.infer else 1, self.c_maxlen, self.q_maxlen, config["char_limit"], config["hidden"], config["char_dim"], config["num_heads"]
